import { Shield, Mail, Cpu } from 'lucide-react';

export default function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="bg-cyber-dark py-12 border-t border-cyber-border/30 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center md:items-start space-y-2 text-center md:text-left">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-mono text-sm font-bold text-white tracking-wider">
              ANAND K <span className="text-cyber-green">//</span> SEC
            </span>
          </div>
          <p className="text-slate-500 font-mono text-[10px] tracking-wide">
            &copy; {currentYear} Anand K. All digital assets encrypted.
          </p>
        </div>

        {/* Diagnostic indicator block */}
        <div className="flex items-center space-x-6 text-slate-500 font-mono text-[9px] uppercase tracking-wider select-none">
          <div className="flex items-center space-x-1.5">
            <Cpu className="w-3.5 h-3.5 text-cyber-green animate-pulse" />
            <span>SYS_INTEGRITY: SECURE</span>
          </div>
          <span className="text-slate-800">|</span>
          <div>LATENCY: 12ms</div>
          <span className="text-slate-800">|</span>
          <div>SSL_HANDSHAKE: PASS</div>
        </div>
